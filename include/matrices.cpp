...
{
	if(currentMatrix==MODEL_MATRIX)
		modelMatrix[modelMatrix.size() - 1] *= glm::rotate(degreesToRadians(angle), glm::vec3(1.0f, 0.0f, 0.0f));	//f!
	else if(currentMatrix==VIEW_MATRIX) //camera
		viewMatrix[viewMatrix.size() - 1] *= glm::rotate(-degreesToRadians(angle), glm::vec3(1.0f, 0.0f, 0.0f));
...
{
	if(currentMatrix==MODEL_MATRIX)
		modelMatrix[modelMatrix.size() - 1] *= glm::rotate(degreesToRadians(angle), glm::vec3(0.0f, 1.0f, 0.0f));	//f!
	else if(currentMatrix==VIEW_MATRIX) //camera
		viewMatrix[viewMatrix.size() - 1] *= glm::rotate(-degreesToRadians(angle), glm::vec3(0.0f, 1.0f, 0.0f));
...
{
	if(currentMatrix==MODEL_MATRIX)
		modelMatrix[modelMatrix.size() - 1] *= glm::rotate(degreesToRadians(angle), glm::vec3(0.0f, 0.0f, 1.0f));	//f!
	else if(currentMatrix==VIEW_MATRIX) //camera
		viewMatrix[viewMatrix.size() - 1] *= glm::rotate(-degreesToRadians(angle), glm::vec3(0.0f, 0.0f, 1.0f));
...
{
	projectionMatrix[projectionMatrix.size()-1]=glm::perspective(degreesToRadians(angle),aRatio,n,f);
	matricesReady=false;
}
...
glm::mat3& matrices::getNormalMatrix()
{
	...
	if (!matricesReady)
		normalMatrix = glm::transpose(glm::inverse(glm::mat3(getModelViewMatrix())));
